...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("Fourtwenty", [], factory);
	else if(typeof exports === 'object')
		exports["Fourtwenty"] = factory();
	else
		root["Fourtwenty"] = factory();
})(this, function() {
        ...
        query.getFilterChanges(self.filterId, function (changeError, changeResult) {
          ...
          if (!changeError) {
            try {
              ...
              decodingError = new Error('[fourtwozerojs-filter] while decoding filter change event data from RPC \'' + JSON.stringify(decodedChangeResults) + '\': ' + decodingErrorMesage);
            }
          }
...
/**
 * FourtwentyFilter constructor, intakes a query, helps manage filter event polling
 *
 * @method FourtwentyFilter
 * @param {Object} query the `fourtwozerojs-query` or `fourtwenty-query` object
 * @returns {Object} output an FourtwentyFilter instance
 * @throws error if new is not used
 */

function FourtwentyFilter(query) {
  var self = this;
  if (!(self instanceof FourtwentyFilter)) {
    throw new Error('the FourtwentyFilter object must be instantiated with `new` flag.. (e.g. `const filters = new FourtwentyFilter(query);`)');
  }
  if (typeof query !== 'object') {
    throw new Error('the FourtwentyFilter object must be instantiated with an FourtwentyQuery instance (e.g. `const filters = new FourtwentyFilter(new FourtwentyQuery(provider));`). See github.com/420integrated/fourtwozerojs-query for more details..');
  }
...
// export FourtwentyFilter
module.exports = FourtwentyFilter;
...
var FourtwentyQuery = __webpack_require__(19);
var FourtwentyFilter = __webpack_require__(6);
var FourtwentyContract = __webpack_require__(14);
...
function Fourtwenty(cprovider, options) {
  if (!(this instanceof Fourtwenty)) {
    throw new Error('[fourtwozerojs] the Fourtwenty object requires you construct it with the "new" flag (i.e. `const fourtwenty = new Fourtwenty(...);`).');
  }
  var self = this;
  self.options = options || {};
  var query = new FourtwentyQuery(cprovider, self.options.query);
...
Fourtwenty.BN = BN;
Fourtwenty.isAddress = function (val) {
  return utils.isHexString(val, 20);
};
Fourtwenty.keccak256 = function (val) {
...
module.exports = function (fourtwenty) {
  return function (txHash, callback) {
    var count = 0;

    var timeout = fourtwenty.options.timeout || 800000;
    var interval = fourtwenty.options.interval || 7000;

    var prom = new Promise(function (resolve, reject) {
      var txInterval = setInterval(function () {
        fourtwenty.getTransactionReceipt(txHash, function (err, result) {
          if (err) {
...
function encodeParams(types, values) {
  if (types.length !== values.length) {
    throw new Error('[fourtwozerojs-abi] while encoding params, types/values mismatch, Your contract requires ' + types.length + ' types (arguments), and you passed in ' + values.length);
  }
...
function hexOrBuffer(valueInput, name) {
  ...
  if (!Buffer.isBuffer(value)) {
    if (!isHexString(value)) {
      var error = new Error(name ? '[fourtwozerojs-abi] invalid ' + name : '[fourtwozerojs-abi] invalid hex or buffer, must be a prefixed alphanumeric even length hex string');
      error.reason = '[fourtwozerojs-abi] invalid hex string, hex must be prefixed and alphanumeric (e.g. 0x023..)';
      error.value = value;
      throw error;
    }
...
function getKeys(params, key, allowEmpty) {
  ...
  if (!Array.isArray(params)) {
    throw new Error('[fourtwozerojs-abi] while getting keys, invalid params value ' + JSON.stringify(params));
  }
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    if (allowEmpty && !value) {
      ...
      throw new Error('[fourtwozerojs-abi] while getKeys found invalid ABI data structure, type value not string');
    }
    result.push(value);
  }
  ...
  return {
    ...
    decode: function decodeFixedBytes(data, offset) {
      if (data.length !== 0 && data.length < offset + 32) {
        throw new Error('[fourtwozerojs-abi] while decoding fixed bytes, invalid bytes data length: ' + length);
      }
...
var coderAddress = {
  encode: function encodeAddress(valueInput) {
    ...
    if (!isHexString(value, 20)) {
      throw new Error('[fourtwozerojs-abi] while encoding address, invalid address value, not alphanumeric 20 byte hex string');
    }
    value = hexOrBuffer(value);
    result.fill(0);
  ...
  decode: function decodeAddress(data, offset) {
    ...
    if (data.length !== 0 && data.length < offset + 32) {
      throw new Error('[fourtwozerojs-abi] while decoding address data, invalid address data, invalid byte length ' + data.length);
    }
    return {
      consumed: 32,
...
function decodeDynamicBytesHelper(data, offset) {
  if (data.length !== 0 && data.length < offset + 32) {
    throw new Error('[fourtwozerojs-abi] while decoding dynamic bytes data, invalid bytes length: ' + data.length + ' should be less than ' + (offset + 32));
  }

  var length = uint256Coder.decode(data, offset).value; // eslint-disable-line
  length = length.toNumber();
  if (data.length !== 0 && data.length < offset + 32 + length) {
    throw new Error('[fourtwozerojs-abi] while decoding dynamic bytes data, invalid bytes length: ' + data.length + ' should be less than ' + (offset + 32 + length));
  }
  ...
  return {
    encode: function encodeArray(value) {
      ...
      if (!Array.isArray(value)) {
        throw new Error('[fourtwozerojs-abi] while encoding array, invalid array data, not type Object (Array)');
      }
      ...
      if (length !== value.length) {
        throw new Error('[fourtwozerojs-abi] while encoding array, size mismatch array length ' + length + ' does not equal ' + value.length);
      }
...
function getParamCoder(typeInput) {
  ...
  var invalidTypeErrorMessage = '[fourtwozerojs-abi] while getting param coder (getParamCoder) type value ' + JSON.stringify(type) + ' is either invalid or unsupported by fourtwozerojs-abi.';
    ...
    switch (prefix) {
      case 'int':case 'uint':
        ...
        if (intSize === 0 || intSize > 256 || intSize % 8 !== 0) {
          throw new Error('[fourtwozerojs-abi] while getting param coder for type ' + type + ', invalid ' + prefix + '<N> width: ' + type);
        }
      ...
      case 'bytes':
        ...
        if (part[3]) {
          ...
          if (size === 0 || size > 32) {
            throw new Error('[fourtwozerojs-abi] while getting param coder for prefix bytes, invalid type ' + type + ', size ' + size + ' should be 0 or greater than 32');
          }
...
var FourtwentyFilter = __webpack_require__(6); // eslint-disable-line
...
function hasTransactionObject(args) {
  var txObjectProperties = ['from', 'to', 'data', 'value', 'smokePrice', 'smoke'];
...
function contractFactory(query) {
  return function ContractFactory(contractABI, contractBytecode, contractDefaultTxObject) {
    if (!Array.isArray(contractABI)) {
      throw new Error('[fourtwozerojs-contract] Contract ABI must be type Array, got type ' + typeof contractABI);
    }
    if (typeof contractBytecode !== 'undefined' && typeof contractBytecode !== 'string') {
      throw new Error('[fourtwozerojs-contract] Contract bytecode must be type String, got type ' + typeof contractBytecode);
    }
    if (typeof contractDefaultTxObject !== 'undefined' && typeof contractDefaultTxObject !== 'object') {
      throw new Error('[fourtwozerojs-contract] Contract default tx object must be type Object, got type ' + typeof contractABI);
    }
    ...
    output.at = function atContract(address) {
      function Contract() {
        ...
        self.filters = new FourtwentyFilter(query);
                ...
                function newMethodCallback(callbackError, callbackResult) {
                  if (queryMethod === 'call' && !callbackError) {
                    try {
                      ...
                      var decodingError = new Error('[fourtwozerojs-contract] while formatting incoming raw call data ' + JSON.stringify(callbackResult) + ' ' + decodeFormattingError);
...
function FourtwentyContract(query) {
  return contractFactory(query);
}

module.exports = FourtwentyContract;
...
function formatQuantity(value, encode) {
  ...
  if (numberToBN(value).isNeg()) {
    throw new Error('[fourtwozerojs-format] while formatting quantity \'' + numberValue.toString(10) + '\', invalid negative number. Number must be positive or zero.');
  }
...
function formatData(value, byteLength) {
  ...
  && (!/^[0-9A-Fa-f]+$/.test(stripHexPrefix(output)) || outputByteLength !== 2 + byteLength * 2)) {
    throw new Error('[fourtwozerojs-format] hex string \'' + output + '\' must be an alphanumeric ' + (2 + byteLength * 2) + ' utf8 byte hex (chars: a-fA-F) string, is ' + outputByteLength + ' bytes');
  }
 ...
 *
 * @method formatObject
 * @param {String|Array} formatter the unit to convert to, default 420coin
...
function formatObject(formatter, value, encode) {
  ...
  if (typeof formatter === 'string') {
    if (formatter === 'Boolean|FourtwentySyncing') {
      formatObject = Object.assign({}, schema.objects.FourtwentySyncing);
  ...
  if (!arrayContainsArray(Object.keys(value), formatObject.__required)) {
    // eslint-disable-line
    throw new Error('[fourtwozerojs-format] object ' + JSON.stringify(value) + ' must contain properties: ' + formatObject.__required.join(', ')); // eslint-disable-line
  }
 ...
 *
 * @method formatArray
 * @param {String|Array} formatter the unit to convert to, default 420coins
...
function invalidResponseError(request, host) {
  var responseError = new Error('[fourtwozerojs-provider-http] Invalid JSON RPC response from provider\n    host: ' + host + '\n    response: ' + String(request.responseText) + ' ' + JSON.stringify(request.responseText, null, 2) + '\n    responseURL: ' + request.responseURL + '\n    status: ' + request.status + '\n    statusText: ' + request.statusText + '\n  ');
...
function HttpProvider(host, timeout) {
  if (!(this instanceof HttpProvider)) {
    throw new Error('[fourtwozerojs-provider-http] the HttpProvider instance requires the "new" flag in order to function normally (e.g. `const fourtwenty = new Fourtwenty(new HttpProvider());`).');
  }
  if (typeof host !== 'string') {
    throw new Error('[fourtwozerojs-provider-http] the HttpProvider instance requires that the host be specified (e.g. `new HttpProvider("http://localhost:6174")` or via service like infura `new HttpProvider("http://ropsten.infura.io")`)');
  }
...
HttpProvider.prototype.sendAsync = function (payload, callback) {
  ...
  request.ontimeout = function () {
    callback('[fourtwozerojs-provider-http] CONNECTION TIMEOUT: http request timeout after ' + self.timeout + ' ms. (i.e. your connect has timed out for whatever reason, check your provider).', null);
  };

  try {
    request.send(JSON.stringify(payload));
  } catch (error) {
    callback('[fourtwozerojs-provider-http] CONNECTION ERROR: Couldn\'t connect to node \'' + self.host + '\': ' + JSON.stringify(error, null, 2), null);
  }
};
...
var FourtwentyRPC = __webpack_require__(20);

module.exports = Fourtwenty;

function Fourtwenty(provider, options) {
  var self = this;
  var optionsObject = options || {};

  if (!(this instanceof Fourtwenty)) {
    throw new Error('[fourtwozerojs-query] the Fourtwenty object requires the "new" flag in order to function normally (i.e. `const fourtwenty = new Fourtwenty(provider);`).');
  }
  if (typeof provider !== 'object') {
    throw new Error('[fourtwozerojs-query] the Fourtwenty object requires that the first input \'provider\' must be an object, got \'' + typeof provider + '\' (i.e. \'const fourtwenty = new Fourtwenty(provider);\')');
  }
  ...
  self.rpc = new FourtwentyRPC(provider);
  self.setProvider = self.rpc.setProvider;
}

Fourtwenty.prototype.log = function log(message) {
  var self = this;
  if (self.options.debug) self.options.logger.log('[fourtwozerojs-query log] ' + message);
};

Object.keys(format.schema.methods).forEach(function (rpcMethodName) {
  Object.defineProperty(Fourtwenty.prototype, rpcMethodName.replace('fourtwenty_', ''), {
    enumerable: true,
...
function generateFnFor(method, methodObject) {
  return function outputMethod() {
    ...
    var self = this;
    var args = [].slice.call(arguments); // eslint-disable-line
    var protoMethod = method.replace('fourtwenty_', ''); // eslint-disable-line
    ...
    var prom = new Promise(function (resolve, reject) {
      var cb = function cb(callbackError, callbackResult) {
        if (callbackError) {
          ...
          try {
            ...
            var outputError = new Error('[fourtwozerojs-query] while formatting outputs from RPC \'' + JSON.stringify(callbackResult, null, self.options.jsonSpace) + '\' for method \'' + protoMethod + '\' ' + outputFormattingError);
      ...
      if (args.length < methodObject[2]) {
        return cb(new Error('[fourtwozerojs-query] method \'' + protoMethod + '\' requires at least ' + methodObject[2] + ' input (format type ' + methodObject[0][0] + '), ' + args.length + ' provided. For more information visit: https://github.com/ethereum/wiki/wiki/JSON-RPC#' + method.toLowerCase()));
      }

      if (args.length > methodObject[0].length) {
        return cb(new Error('[fourtwozerojs-query] method \'' + protoMethod + '\' requires at most ' + methodObject[0].length + ' params, ' + args.length + ' provided \'' + JSON.stringify(args, null, self.options.jsonSpace) + '\'. For more information visit: https://github.com/ethereum/wiki/wiki/JSON-RPC#' + method.toLowerCase()));
      }
      ...
      try {
        ...
        return cb(new Error('[fourtwozerojs-query] while formatting inputs \'' + JSON.stringify(args, null, self.options.jsonSpace) + '\' for method \'' + protoMethod + '\' error: ' + formattingError));
      }
...
module.exports = FourtwentyRPC;

/**
 * Constructs the FourtwentyRPC instance
 *
 * @method FourtwentyRPC
 * @param {Object} cprovider the fourtwenty rpc provider web3 standard..
 * @param {Object} options the options, if any
 * @returns {Object} fourtwentyrpc instance
 */
function FourtwentyRPC(cprovider, options) {
  var self = this;
  var optionsObject = options || {};

  if (!(this instanceof FourtwentyRPC)) {
    throw new Error('[fourtwozerojs-rpc] the FourtwentyRPC object requires the "new" flag in order to function normally (i.e. `const fourtwenty = new FourtwentyRPC(provider);`).');
  }
  ...
  self.setProvider = function (provider) {
    if (typeof provider !== 'object') {
      throw new Error('[fourtwozerojs-rpc] the FourtwentyRPC object requires that the first input \'provider\' must be an object, got \'' + typeof provider + '\' (i.e. \'const fourtwenty = new FourtwentyRPC(provider);\')');
    }
 ...
 */
FourtwentyRPC.prototype.sendAsync = function sendAsync(payload, callback) {
  var self = this;
  ...
  var promise = new Promise(function (resolve, reject) {
    self.currentProvider.sendAsync(parsedPayload, function (err, response) {
      ...
      if (err || responseObject.error) {
        var payloadErrorMessage = '[fourtwozerojs-rpc] ' + (responseObject.error && 'rpc' || '') + ' error with payload ' + JSON.stringify(parsedPayload, null, self.options.jsonSpace) + ' ' + (err ? String(err) : JSON.stringify(responseObject.error, null, self.options.jsonSpace));
...
// complete 420coin unit map
var unitMap = {
  'no420coin': '0', // eslint-disable-line
  'marley': '1', // eslint-disable-line
  'kmarley': '1000', // eslint-disable-line
  'Kmarley': '1000', // eslint-disable-line
  'woody': '1000', // eslint-disable-line
  'femto420': '1000', // eslint-disable-line
  'mmarley': '1000000', // eslint-disable-line
  'Mmarley': '1000000', // eslint-disable-line
  'rogen': '1000000', // eslint-disable-line
  'pico420': '1000000', // eslint-disable-line
  'gmarley': '1000000000', // eslint-disable-line
  'Gmarley': '1000000000', // eslint-disable-line
  'maher': '1000000000', // eslint-disable-line
  'nano420': '1000000000', // eslint-disable-line
  'nano': '1000000000', // eslint-disable-line
  'snoop': '1000000000000', // eslint-disable-line
  'micro420': '1000000000000', // eslint-disable-line
  'micro': '1000000000000', // eslint-disable-line
  'willie': '1000000000000000', // eslint-disable-line
  'milli420': '1000000000000000', // eslint-disable-line
  'milli': '1000000000000000', // eslint-disable-line
  '420coin': '1000000000000000000', // eslint-disable-line
  'k420coin': '1000000000000000000000', // eslint-disable-line
  'cheech': '1000000000000000000000', // eslint-disable-line
  'chong': '1000000000000000000000000', // eslint-disable-line
  'spicoli': '1000000000000000000000000000000000000000000', // eslint-disable-line
 ...
 * @returns {BigNumber} value of the unit (in Marley)
 * @throws error if the unit is not correct:w
 */
function getValueOfUnit(unitInput) {
  var unit = unitInput ? unitInput.toLowerCase() : '420coin';
  var unitValue = unitMap[unit]; // eslint-disable-line

  if (typeof unitValue !== 'string') {
    throw new Error('[fourtwozerojs-unit] the unit provided ' + unitInput + ' doesn\'t exists, please use the one of the following units ' + JSON.stringify(unitMap, null, 2));
  }
...
function fromMarley(marleyInput, unit, optionsInput) {
  var marley = numberToBN(marleyInput); // eslint-disable-line
  var negative = marley.lt(zero); // eslint-disable-line
  var base = getValueOfUnit(unit);
  var baseLength = unitMap[unit].length - 1 || 1;
  var options = optionsInput || {};

  if (negative) {
    marley = marley.mul(negative1);
  }

  var fraction = marley.mod(base).toString(10); // eslint-disable-line
  ...
  var whole = marley.div(base).toString(10); // eslint-disable-line
...
function toMarley(fourtwentyerInput, unit) {
  var fourtwentycoin = numberToString(fourtwentyerInput); // eslint-disable-line
  var base = getValueOfUnit(unit);
  var baseLength = unitMap[unit].length - 1 || 1;

  // Is it negative?
  var negative = fourtwentycoin.substring(0, 1) === '-'; // eslint-disable-line
  if (negative) {
    ether = fourtwentycoin.substring(1);
  }

  if (fourtwentycoin === '.') {
    throw new Error('[fourtwozerojs-unit] while converting number ' + fourtwentyInput + ' to marley, invalid value');
  }

  // Split it into a whole and fractional part
  var comps = ether.split('.'); // eslint-disable-line
  if (comps.length > 2) {
    throw new Error('[fourtwozerojs-unit] while converting number ' + fourtwentyInput + ' to marley,  too many decimal points');
  }
  ...
  if (fraction.length > baseLength) {
    throw new Error('[fourtwozerojs-unit] while converting number ' + fourtwentyInput + ' to marley, too many decimal places');
  }
  ...
  var marley = whole.mul(base).add(fraction); // eslint-disable-line

  if (negative) {
    marley = marley.mul(negative1);
  }

  return new BN(marley.toString(10), 10);
}
...
module.exports = {
	"methods": {
		...
		"fourtwenty_blockNumber": [
			[],
			"Q"
		],
		"fourtwenty_getBalance": [
			[
				"D20",
				"Q|T"
			],
		...
		"fourtwenty_getTransactionCount": [
			[
				"D20",
				"Q|T"
			],
		...
		"fourtwenty_getBlockTransactionCountByHash": [
			[
				"D32"
			],
			"Q",
		...
		"fourtwenty_getBlockTransactionCountByNumber": [
			[
				"Q|T"
			],
			"Q",
		...
		"fourtwenty_getUncleCountByBlockHash": [
			[
				"D32"
			],
			"Q",
		...
		"fourtwenty_getUncleCountByBlockNumber": [
			[
				"Q"
			],
			"Q",
		...
		"fourtwenty_getTransactionByBlockHashAndIndex": [
			[
				"D32",
				"Q"
			],
		...
		"fourtwenty_getTransactionByBlockNumberAndIndex": [
			[
				"Q|T",
				"Q"
			],
		...
		"fourtwenty_getTransactionReceipt": [
			[
				"D32"
			],
			"Receipt",
		...
		"fourtwenty_getUncleByBlockHashAndIndex": [
			[
				"D32",
				"Q"
			],
		...
		"fourtwenty_getUncleByBlockNumberAndIndex": [
			[
				"Q|T",
				"Q"
			],
		...
		"fourtwenty_getCompilers": [
			[],
			[
				"S"
			]
		],
		"fourtwenty_compileLLL": [
			[
				"S"
			],
			"D",
...
;
//# sourceMappingURL=fourtwozerojs.js.map